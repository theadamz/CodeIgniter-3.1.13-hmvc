...
class REST_Controller extends \MY_Controller
{
    ...
    public function __construct($config = 'rest')
    {
        ...
        if ($this->request->format && $this->request->body) {
            $this->request->body = Format_rest::factory($this->request->body, $this->request->format)->to_array();
    ...
    public function response($data = null, $http_code = null, $continue = false)
    {
        ...
        if (!$isProfilingEnabled) {
            ...
            elseif ($data !== null) {
                ...
                if (method_exists(Format_rest::class, 'to_'.$this->response->format)) {
                    // CORB protection
                    // First, get the output content.
                    $output = Format_rest::factory($data)->{'to_'.$this->response->format}();
                    ...
                    if ($this->response->format === 'array') {
                        $output = Format_rest::factory($output)->{'to_json'}();
                    }
                } else {
                    // If an array or object, then parse as a json, so as to be a 'string'
                    if (is_array($data) || is_object($data)) {
                        $data = Format_rest::factory($data)->{'to_json'}();
                    }
